...
const formValidityCheck = event =>{
  ...
  return (
    ...
    <input type="password" className="form-control" onChange={passChangeHandler}/>
    {!enteredPasswordIsValid && (<p className="text-danger">رمز عبور نباید خالی باشد</p>)}
    <label >تکرار رمز عبور</label>
    <input type="password" className="form-control" onChange={secondpassChangeHandler}/ >
    {!enteredSecpassIsValid && (<p className="text-danger">این فیلد نباید خالی باشد</p>)}

    {!passwordmatch && (<p className="text-danger">تکرار رمز عبور با اصل رمز عبور همخوانی ندارد</p>)}
